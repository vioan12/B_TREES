...
NodeB *insert(int key)
{              // insert key read in the B tree and will return the corresponding root pointer
...
void splitPage(NodeB *node,int keyX,int &upKey,int &ok)
{   // search keyX in the current page node and inserted in place of corresponding
      NodeB *adr;
      int ind,found;
      int position,new1;
...
void placeInPage(NodeB *node,int position,int keyX,int &upKey,int &ok)
{   //actually insert the keyX in the page node in the position position of of keys vector
    NodeB *pv;
     int key;
     int halfm;
     halfm=node->nr/2;
...
void insertInNonFullLeaf(NodeB *node,int position,int keyX,int &upKey,int &ok)
{   //enter in the current node
          NodeB *adr;
          int ind;
          ok=0;
          node->nr=node->nr+1;
...
void search(NodeB *nodex,int numtosearch)
...
void parcurgere1(NodeB *node ,int level){   //display the tree
       int j;
       if(node!=NULL){
                       if(level>level_max)
                            level_max=level;
                        for(j=0;j<=node->nr;j++)
                            parcurgere1(node->pNodes[j],level+1);
       }
}

void parcurgere2(NodeB *node ,int level){   //display the tree
       int j;
       if(node!=NULL){
                        if(level_max==level)
                            add_node(node);
                        for(j=0;j<=node->nr;j++)
                            parcurgere2(node->pNodes[j],level+1);
       }
}
...
NodeB *deleteKey(NodeB **nod,int key)
{      // delete a key
     NodeB *temp;
     int i=1,j,nr=(*nod)->nr;
     NodeB *child=NULL;
...
void afisare()
{
    ...
    printf("\n");
    while(p!=NULL){
        for(i=1 ;i<=p->val->nr;i++)
            printf("%d ",p->val->keys[i]);
        p=p->next;
    }
}
